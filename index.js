...
function stats(){
  ...
  if(config.screeps.perTick)
    return statsPerTick()
  else
    return statsSingleTick()
}

function statsSingleTick(){
  return api.memory.get('stats').then(stats=>{
    return saveStats([{
      time: Date.now(),
      stats: stats
    }])
...
function saveStats(spt){
  if(config.showRawStats) console.log('STATS',spt)
  let payload = []
  spt.forEach(s=>{
